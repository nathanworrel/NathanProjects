...
class ImageStandardizer(object):
    ...
    def fit(self, X):
        ...
        g = X[:,:,:,1]
        b = X[:,:,:,2]
        self.image_mean = np.array([np.mean(r),np.mean(g),np.mean(b)])
        self.image_std = np.array([np.std(r),np.std(g),np.std(b)])

    def transform(self, X):
        """Return standardized dataset given dataset X."""
        # TODO: Complete this function
        X_transformed = (X - self.image_mean) / self.image_std
        #print([[np.mean(X_transformed[:,:,:,0]),np.mean(X_transformed[:,:,:,1]),np.mean(X_transformed[:,:,:,2])]])
