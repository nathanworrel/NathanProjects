...
class RLAgent:
    ...
    def __init__(self, grid, discount_factor, learning_rate, episodes):
        ...
        self.current_state = grid.states[grid.start]
        self.q = {}
        self.n = {}
        for i in grid.states:
            for j in i:
                for k in j.get_actions():
                    self.q[j.position, k] = 0
                    self.n[j.position, k] = 0
    ...
    def q_learning(self):
        ...
        for i in range(self.eps):
            ...
            while not self.current_state.terminal:
                ...
                self.n[self.current_state.position, action] += 1
                maximum = float('-inf')
                for i in self.current_state.get_actions():
                    if self.q[new_state.position, i] > maximum:
                        maximum = self.q[new_state.position, i]
                self.q[self.current_state.position, action] = self.q[self.current_state.position, action] + self.lr*(reward + self.df*maximum - self.q[self.current_state.position, action])
    ...
    def SARSA(self):
        ...
        for i in range(self.eps):
            ...
            while not self.current_state.terminal:
                new_state, reward = self.execute_action(action)
                action_prime = self.select_action(new_state)
                self.n[self.current_state.position, action] += 1
                self.q[self.current_state.position, action] = self.q[self.current_state.position, action] + self.lr*(reward + self.df*self.q[new_state.position, action_prime] - self.q[self.current_state.position, action])
                self.current_state = new_state
                action = action_prime
        return self.q
